---
title: 站点
date: 2020-12-15 01:12:40
---

## 工具

- [Fuun.fun - 有趣的网页小程序](https://fuun.fun/)
- [i18ns - 国际化文案](https://i18ns.com/zh/index.html)
- [Regex101 - 可视化正则](https://regex101.com/)
- [crontab.guru - 定时任务可视化](https://crontab.guru/)
- [TableConvert - 在线表格编辑器](https://tableconvert.com/)
- [uiGradients -生成渐变色](https://uigradients.com/)
- [WebGradients - 生成渐变色](https://webgradients.com/)
- [CSSFx - 可复制的 CSS 特效](https://cssfx.dev/)
- [pattern-css - 背景图 CSS](https://bansal.io/pattern-css)
- [heropatterns - 背景图 CSS](http://www.heropatterns.com/)
- [98.css - Win98 风 CSS](https://jdan.github.io/98.css/)
- [Neumorphism - 实现新拟态效果](https://neumorphism.io/)
- [carbon - 生成代码图片](https://carbon.now.sh/)
- [DownGit - Github 资源打包下载](https://yehonal.github.io/DownGit/#/home)
- [enjoycss - CSS 生成器](https://enjoycss.com/)
- [Coding Fonts - 编程字体预览](https://coding-fonts.css-tricks.com/)
- [NGINX 在线配置](https://www.digitalocean.com/community/tools/nginx)

## 资源

- [编程导航](https://www.code-nav.cn)
- [ThemeLock - 模板库](http://www.themelock.com)
- [bootstrapmb - 模板库](http://www.bootstrapmb.com/muban/houtai)
- [云核 - 模板库](http://www.yunnut.com/)
- [FTUApps.Dev - Crack 软件](https://ftuapps.dev/)
- [SaltTiger - 电子书](https://salttiger.com/)
- [ZLibrary.China - 电子书](https://b-ok.as/)
- [Course Club -IT 课程](https://courseclub.net/)
- [云盘精灵 - 云盘搜索](https://www.yunpanjingling.com/)
- [磁力链 BT - BT 站点](http://cililianbt.pw/)
- [audioz - 音乐资源](https://audioz.download/)
- [IT 资源整合](https://www.yeahhub.com/)
- [BT 磁力搜索引擎索引-在线](https://hao.su/909/)
- [磁力搜 - 磁力聚合搜索](https://biedian.me/)
- [Sketch App Sources - Sketch 设计资源](https://www.sketchappsources.com/)
- [Awesome Public Datasets - 公共数据](https://github.com/awesomedata/awesome-public-datasets)
- [Public APIs - 公共接口](https://github.com/public-apis/public-apis)
- [程序员找工作黑名单](https://github.com/shengxinjing/programmer-job-blacklist)
- [合集网](https://www.heji.ltd/)
- [掘金翻译计划](https://github.com/xitu/gold-miner)

## Blog

- [Dan Abramov](https://overreacted.io/)
- [Robin Wieruch](https://www.robinwieruch.de/)
- [LucasHC](https://www.jianshu.com/u/452568260db5)
- [Lydia Hallie](https://dev.to/lydiahallie)
- [Samantha Ming](https://www.samanthaming.com/)

## 社区

- [DEV](https://dev.to/)
- [掘金](https://juejin.im/)

## CDN

- [cdnjs](https://cdnjs.com/)
- [UNPKG](https://unpkg.com/)
- [jsDelivr – Open Source CDN](https://www.jsdelivr.com/)

## 动漫

- [ACG.RIP](https://acg.rip)
- [ZzzFun 动漫视频网](http://www.zzzfun.com/)
- [关于浪漫、讽刺、数学和语言的网络漫画](https://xkcd.com/)

## 其他

- [Photo505](http://photo505.com/)
- [电影剧照](https://film-grab.com/)
- [像素画社区](http://codeplay.buzz/)
- [贪吃蛇](https://playsnake.org/)
- [Treeographer](https://thetreeographer.com/archive/)
- [此人不存在](https://thispersondoesnotexist.com/)
- [国际空间站](https://www.google.com/maps/@29.5602853,-95.0853914,2a,75y,212.04h,90t/data=!3m7!1e1!3m5!1szChzPIAn4RIAAAQvxgbyEg!2e0!3e5!7i10000!8i5000?shorturl=1)
- [地球时间线](http://timelineofearth.com/)
- [Yes or No](https://yesno.wtf/)
- [学音乐](https://learningmusic.ableton.com/zh-Hans/)
- [小霸王](https://www.yikm.net/)
- [无限分裂](http://koalastothemax.com/)
- [能不能好好说话？](https://lab.magiconch.com/nbnhhsh/)
- [AI 图片合成](https://artbreeder.com/)
- [不会百度么](http://buhuibaidu.me/)
- [蒸気機](https://lab.magiconch.com/vaporwave/)
- [Geek prank](https://geekprank.com/)
- [快速涂鸦](https://quickdraw.withgoogle.com/)
- [Bellwoods](https://bellwoods.xyz/)
- [视频变漫画](https://comixify.ai/)
- [开源人物肖像](http://www.facesofopensource.com/)
- [核弹威力](https://nuclearsecrecy.com/nukemap/)
- [卦象二维码](https://iching.codes/)
- [漫威 API](https://developer.marvel.com/)
- [黑白照片上色](https://colourise.sg/)
- [频闪幻觉](https://strobe.cool/)
- [井字棋](https://playtictactoe.org/)
- [全景故宫](https://pano.dpm.org.cn/)
- [木材数据库](https://www.wood-database.com/)
- [遥控车](https://bruno-simon.com/)
- [太鼓达人](https://taiko.bui.pm/)
- [自由钢琴](https://www.autopiano.cn/)
- [索尼风景图](https://www.sony.net/united/clock/)
- [濒临灭绝的动物](https://www.webhek.com/apps/species-in-pieces/)
- [斯诺克](http://www.heyzxz.me/pcol/)
- [俄罗斯方块](https://chvin.github.io/react-tetris/)
- [PornHub Logo](https://logoly.pro/)
- [P 站](https://pixivic.com/)
- [毒鸡汤](https://8zt.cc/)
- [图片粒子效果](https://codepen.io/zadvorsky/pen/PNXbGo)
- [9 GANS](https://9gans.com/)
- [流星雨](https://www.meteorshowers.org/)
- [Emoji 游戏](http://emojisandearthporn.com/)
- [找到隐藏的牛](https://findtheinvisiblecow.com/)
- [知妖（中国妖怪百集）](http://www.cbaigui.com/)
- [天际网络摄像头](https://www.skylinewebcams.com/)
- [深海](https://neal.fun/deep-sea/)
- [在线 AI 图像处理](https://photo.opencool.cn/)
- [自我销毁网站](https://www.thiswebsitewillselfdestruct.com/)
- [在线 DOS 游戏](https://dos.zczc.cz/)
- [图片隐写术](https://c.p2hp.com/yinxietu/)
- [Link Lock](https://jstrieb.github.io/link-lock)
- [猫狗还是情人](https://yanhaixiang.com/cat-dog-lover/)
- [傻逼网](https://is.sb/)
- [诺基亚短信图片生成器](https://zzkia.noddl.me/)
- [带你去无用的网站](https://theuselessweb.com/)
- [节奏照片](http://nodtotherhythm.com/)
- [Pixel Pin](https://www.pixelpin.io/)
- [Image Charts](https://www.image-charts.com/)
- [网站设计博物馆](https://www.webdesignmuseum.org/)
- [机械键盘博物馆](https://scrapbox.io/MECHKEYS/)
- [电台公园](http://radio.garden/)
- [Gource](https://gource.io/)
- [CSS 油画](https://diana-adrianne.com/purecss-lace/)
- [物流艺术](https://logisticsartproject.com/)
- [全历史](https://www.allhistory.com/)
- [狐狸照片](https://foxrudor.de/)
- [极像素](https://www.sigoo.com/)
- [骚](http://www.sao.com/)
- [CMD.TO](https://cmd.to/)
- [零宽字符隐藏加密](http://zero.rovelast.com/)
- [Picular](https://picular.co/)
- [我要分享](https://fuun.fun/share.html)
